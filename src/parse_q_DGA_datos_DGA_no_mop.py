...
file='/Users/farrospide/Downloads/CaudalesDGA_Coquimbo_diarios_2021.xlsx'

# df=pd.read_csv(file, sep=',', encoding='latin1')

# metadata
metadata= pd.read_excel(file, sheet_name='Ficha_est',
index_col=0)
metadata.index = metadata['rut']
metadata = metadata.drop(columns=['rut'])
...
gdf_nat=gdf[gdf['Estacion'].str.contains('|'.join(stream_names),
case=False)].copy()
...
# data
df_pivot=pd.read_excel(file, sheet_name='Datos',
index_col=0, parse_dates=True)
df_pivot=df_pivot[df_pivot.columns[df_pivot.columns.isin(gdf_nat.index)]]
df_pivot = df_pivot.dropna(how='all')  # Remove completely empty rows
df_pivot = df_pivot[df_pivot.index.year <= 2025]  # Remove future dates
...
station_names = gdf_nat['rut'].tolist()
...
# reset ds_nat index: to UTC, drop tz, normalize, daily freq
# Reset ds_nat index: to UTC, drop tz, normalize, daily freq  

# FIX: Align timezone awareness
# ds_nat.index = pd.to_datetime(ds_nat.index).tz_convert(None)  # ← elimina zona horaria
